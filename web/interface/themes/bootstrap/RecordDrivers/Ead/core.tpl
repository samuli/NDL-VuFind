...
<div id="recordMetadata" class="span8">
  ...
  {* Display Title *}
  <h4 class="alert alert-success recordTitle">{$coreTitle|escape} {if $coreYearRange}({$coreYearRange|escape}){/if}
  ...
  *}
  <span class="label recordInstitution">
    ...
    <tr valign="top" class="recordTags">
      ...
      <td>
        <span style="float:right;">
          <a href="{$url}/Record/{$id|escape:"url"}/AddTag" class="btn btn-success btn-mini tool tagRecord" title="{translate text='Add Tag'}" id="tagRecord{$id|escape}"><i class="icon-plus-sign icon-white"></i>&nbsp;{translate text='Add Tag'}</a>
        </span>
        <div id="tagList">
          {if $tagList}
