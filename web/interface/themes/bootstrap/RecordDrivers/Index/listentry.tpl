...
<td>  
  ...
  <div class="coverDiv">
    <div class="resultNoImage{if $img_count > 0} behindImage{/if}"><p>{translate text='No image'}</p></div>
    {if $img_count > 0}
        <div class="resultImage"><a id="thumbnail_link_{$listId|escape:"url"}" href="{$path}/thumbnail.php?id={$listId|escape:"url"}&index=0&size=large" rel="{$listId|escape:"url"}" onclick="launchFancybox(this); return false;"><img id="thumbnail_{$listId|escape:"url"}" src="{$listThumb|escape}" class="summcover" alt="{translate text='Cover Image'}"/></a></div>
    {else}
