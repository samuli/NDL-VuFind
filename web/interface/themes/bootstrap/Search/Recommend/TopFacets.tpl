...
{if $topFacetSet}
  ...
  <div {if $cluster.label == "Suggested Topics"}id="suggestedTopics" class="span3 pull-right suggestedTopics span-3{if !$sidebarOnLeft} right last{/if}" {else}class="authorbox"{/if}>
    {* $cluster.label *}
    ...
    {foreach from=$cluster.list item=thisFacet name="narrowLoop"}
      ...
      *}
      ...
      {if $thisFacet.isApplied}
        <span class="pull-left">&nbsp;&nbsp;{$thisFacet.value|escape} <img src="{$path}/images/silk/tick.png" alt="Selected"/>&nbsp;</span>
      {else}
        <div class="pull-left facetWrapper"><a href="{$thisFacet.url|escape}">{$thisFacet.value|escape}</a> <span class="facetCount">({$thisFacet.count})</span></div>
      {/if}
