<!-- START of: MetaLib/email.tpl -->

{if $errorMsg}<div class="error">{$errorMsg|translate}</div>{/if}
{if $infoMsg}<div class="info">{$infoMsg|translate}</div>{/if}

<form action="{$url}/MetaLib/Email" method="post" name="emailRecord">
    <input type="hidden" name="url" value="{$searchURL|escape:"html"}"/>
    <input type="hidden" name="type" value="MetaLib"/>
    ...
    <br/>
    <input class="button buttonFinna" type="submit" name="submit" value="{translate text='Send'}"/>
</form>

<!-- END of: MetaLib/email.tpl -->
