...
<div class="well-small myResearch checkedoutList{if $sidebarOnLeft} last{/if}">
  ...
  {if $user->cat_username}
    <span class="lead">{translate text='Your Checked Out Items'}</span>
    {if $blocks}
      ...
      <form name="renewals" action="{$url}/MyResearch/CheckedOut" method="post" id="renewals">
        ...
        <table class="table table-hover">
          <tr>
            ...
            <th>{translate text="adv_search_select_all"}</th>
            <th colspan="1" class="span11 text-right">
              <div class="text-right pull-right floatright">
                <input type="submit" class="btn btn-small btn-success button renew" name="renewSelected" value="{translate text="renew_selected"}" />
                <input type="submit" class="btn btn-small btn-success button renewAll" name="renewAll" value="{translate text='renew_all'}" />
              </div>
            </th>
          </tr>
      {if $errorMsg}
        </table>
        <div class="renewMessage"><p class="alert alert-error">{translate text=$errorMsg}</p></div>
        <table class="table table-hover">
      {/if}
...
-->
        ...
        {foreach from=$transList item=resource name="recordLoop"}
          <tr id="record{$resource.id|escape}" class="result{if ($smarty.foreach.recordLoop.iteration % 2) == 0} alt{/if}">
            ...
            <td class="span9">
              <div class="row-fluid">
                <div class="span8 pull-left resultColumn2">
                ...
                <div class="span3 offset1 dueDate">
                ...
                {if $renewResult[$resource.ils_details.item_id]}
                  {if $renewResult[$resource.ils_details.item_id].success}
                    ...
                    <div class="alert alert-success">{translate text='renew_success'}</div>
                  {else}
                    ...
                    <div class="alert alert-error">{translate text='renew_fail'}{if $renewResult[$resource.ils_details.item_id].sysMessage}: {$renewResult[$resource.ils_details.item_id].sysMessage|escape}{/if}</div>
                  {/if}
                {else}
                  <strong>{translate text='Due Date'}: {$resource.ils_details.duedate|escape} {if $resource.ils_details.dueTime} {$resource.ils_details.dueTime|escape}{/if}</strong>
                  {if $showStatus == "show"}
                    {if $resource.ils_details.dueStatus == "overdue"}
                      <div class="alert alert-error">{translate text="renew_item_overdue"}</div>
                    {elseif $resource.ils_details.dueStatus == "due"}
                      <div class="alert alert-warning notice">{translate text="renew_item_due"}</div>
                    {/if}
                  {/if}
                {/if}

                {if $showStatus == "show" && $resource.ils_details.message}
                  <div class="alert alert-info">{translate text=$resource.ils_details.message}</div>
                {/if}
...
-->
      ...
      {/if}
    {else}
      <div class="clearfix alert alert-info">{translate text='You do not have any items checked out'}.</div>
    {/if}
  {else}
