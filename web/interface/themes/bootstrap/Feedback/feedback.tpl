...
<div class="well-small mainContent">
<h3>{translate text='feedback_heading'}</h3>
{if not $submitted}
{if $errorMsg}<p class="alert alert-error">{$errorMsg}</p>{/if}
<form id="feedbackForm" action="" method="post">
    ...
    <br />
    <input type="submit" id="submit" name="submit" class="btn btn-info button right" value="{translate text='Send'}" />
</form>
<script>
  {literal}
...
{/if}
<p style="margin-top: 2em;"><a href="{$path}/">&laquo; {translate text="To Home"}</a></p>
</div>
