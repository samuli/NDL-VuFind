 ...
 {* Cover image *}
    ...
    {if $img_count == 1}
        <div class="resultImage">
            <a class="title imagePopup" data-id="{$summId|escape:"url"}" href="{$url}/AJAX/JSON?method=getImagePopup&id={$summId|escape:"url"}&index=0&size=large" id="thumbnail_link_{$summId|escape:"url"}" rel="gallery">
    ...
    {elseif $img_count > 1}
    <div class="resultImage">
        <a class="title imagePopup-trigger" href="{$url}/AJAX/JSON?method=getImagePopup&id={$summId|escape:"url"}&index=0&size=large" id="thumbnail_link_{$summId|escape:"url"}">
            <img id="thumbnail_{$summId|escape:"url"}" src="{$path}/thumbnail.php?id={$summId|escape:"url"}&size=small" class="summcover" alt="{translate text='Cover Image'}" />
        </a>
    </div>    
    {/if}
