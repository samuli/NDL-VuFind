...
<div class="sidegroup">
  ...
  {if $sideFacetSet && $recordCount > 0}
    ...
    {else}
        ...
        <dl class="narrowList navmenu{if $cluster.label ==='Main Year'} year{if !empty($visFacets.search_sdaterange_mv[0]) || $filterList.$mainYear} active open collapsed{/if}{/if}{if (is_array($defaultFacets) && in_array($title, $defaultFacets))} open collapsed defaultFacet{/if}">
