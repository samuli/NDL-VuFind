...
<div class="sidegroup">
  ...
  {if in_array('sideFacets', $contextHelp) && $module != 'Collection'}<span id="contextHelp_sideFacets" class="showHelp">{translate text="Search Tips"}</span>{/if}
  ...
  {if $filterList}
    ...
    {foreach from=$filterList item=filters key=field}
      ...
      <li><a href="{$filter.removalUrl|escape}"><span class="roundButton deleteButtonSmall"></span><span class="filterText">{translate text=$field}: {$solrdate|date_format:"%e.%-m.%Y"}—</span></a></li>
          ...
          {else}
        <li><a href="{$filter.removalUrl|escape}"><span class="roundButton deleteButtonSmall"></span><span class="filterText">{if $filter.field != 'building' && !isset($filter.hideField)}{translate text=$field}: {/if}{$filter.display|escape|regex_replace:"/[\[\]]/":""|replace:"-9999":""|replace:"9999":""|replace:" - ":"&ndash;"}</span></a></li>
          {/if}
        {/if}
      {/foreach}
    {/foreach}
  ...
  {if $sideFacetSet && $recordCount > 0}
    {foreach from=$sideFacetSet item=cluster key=title}
      ...
      {else}
          ...
          <dl class="narrowList navmenu{if (($cluster.label ==='Main Year') || ($cluster.label === 'facet_pci_creationdate') || ($cluster.label == 'Published'))} year{if !empty($visFacets.search_sdaterange_mv[0]) || $filterList.$mainYear} open collapsed{/if}{/if}{if (is_array($defaultFacets) && in_array($title, $defaultFacets))} open collapsed defaultFacet{/if}">
          <dt><span class="yearLabel">{translate text=$cluster.label}</span>
             {if $cluster.label == 'Main Year' || $cluster.label == 'facet_pci_creationdate' || $cluster.label == 'Published'}<span class="timelineview">open timeline</span>
