<?php
/**
 * Save action (user list management) for PCI module
 *
 * PHP version 5
 *
    ...
    public function launch()
    {
        ...
        if (!$this->_user) {
            ...
            if (isset($_GET['lightbox'])) {
                ...
                $interface->assign('followupModule', 'PCI');
        ...
        if (isset($_GET['submit'])) {
            $this->saveRecord($this->_user);
            header(
                'Location: ' . $configArray['Site']['url'] . '/PCI/Record?id=' .
                urlencode($_GET['id'])
            );
            exit();
        }
        ...
        $db = ConnectionManager::connectToIndex('PCI');
        ...
        if (isset($_GET['lightbox'])) {
            $interface->assign('title', $_GET['message']);
            return $interface->fetch('PCI/save.tpl');
        } else {
